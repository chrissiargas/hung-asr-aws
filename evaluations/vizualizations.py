...
def plot_layer_fusion_weights(conf, info, weights_per_layer, injection_layers, num_whisper_layers, dataset):
    """
    Plots the pre-extracted layer fusion weights as bar charts.
    """
    if weights_per_layer is None:
        return

    num_injections = len(weights_per_layer)
...
def visualize_random_instance(info, dataset, split='test', num_samples: int = 200):
    ...
    for q in tqdm(range(num_samples), desc="Computing Attention Dynamics"):
        ...
        outputs, cross_attentions, fusion_weights = model.generate(
            audios=audios,
            audio_masks=audio_masks,
            max_new_tokens=200,
            return_attention=True,
            return_layer_fusion=True
        )
        ...
        if q==0:
            ...
            plot_layer_fusion_weights(conf, info, fusion_weights, inj_layers, num_w_layers, dataset)
