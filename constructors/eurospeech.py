...
class eurospeech:
    def __init__(self):
        ...
        self.load_path = "disco-eth/EuroSpeech"
        ...
        if self.conf.language == 'greek':
            self.language = 'greece'

        elif self.conf.language == 'hungarian':
            self.language = 'hu'

    def load_euro_subset(self, split: str):
...
if __name__ == '__main__':
    extractor = eurospeech()
    _ = extractor.load_euro_subset('train')
    _ = extractor.load_euro_subset('validation')
    _ = extractor.load_euro_subset('test')
