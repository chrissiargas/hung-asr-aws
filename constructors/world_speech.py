...
class worldspeech:
    def __init__(self):
        ...
        self.load_path = 'disco-eth/WorldSpeech'
        ...
        # WorldSpeech uses language_country format (BCP-47)
        if self.conf.language == 'greek':
            self.language = 'el_gr'
        elif self.conf.language == 'hungarian':
            self.language = 'hu_hu'

    def load_worldspeech_subset(self, split: str = 'train'):
    ...
    def extract_samples(self, sample, audio_dir: str, manifest_dir: str, idx: int):
        # Audio is provided as a dictionary with 'array' and 'sampling_rate'
        audio_data = sample['audio']['array']
        sampling_rate = sample['audio']['sampling_rate']

        # Resample to your target sampling rate (likely 16kHz based on common ASR tasks)
        ...
        manifest_entry = {
            ...
            'duration': duration,
            'text': sample['human_transcript'],
            'cer': sample['cer'],
            'dns_mos': sample['dnsmos_ovr']
        }
...
if __name__ == '__main__':
    extractor = worldspeech()
    _ = extractor.load_worldspeech_subset('train')
    _ = extractor.load_fleurs_subset('test')